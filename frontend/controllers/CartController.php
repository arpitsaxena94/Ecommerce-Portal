...
class CartController extends \frontend\base\Controller
{
    ...
    {
        \Yii::$app->response->format = \yii\web\Response::FORMAT_HTML; // Ensure response format is HTML
        ...
        if (\Yii::$app->user->isGuest) {
            ...
            foreach ($cartItems as &$item) {
                if ($item['id'] == $id) {
                    $item['quantity']++;
                    $item['total_price'] = $item['quantity'] * $product->price; // Update total price
                    $found = true;
                    break;
                }
            }
            ...
            if (!$found) {
                $cartItem = [
                    ...
                    'price' => $product->price,
                    'quantity' => 1,
                    'total_price' => $product->price // Initialize total price
                ];
                $cartItems[] = $cartItem;
            }
            ...
            if ($cartItem) {
                ...
                $cartItem->quantity = 1;
                $cartItem->total_price = $product->price; // Initialize total price
            }

            if ($cartItem->save()) {
                // Fetch updated items for rendering
                $items = CartItem::getItemsForUser($userId);
                return $this->render('index', [
                    'items' => $items,
                ]);
            } else {
